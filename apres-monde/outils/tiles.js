/* =========================================================
   L'Après-Monde — découpe d'une carte en tuiles (outil local).

   Une carte du monde en un seul fichier devient vite trop
   lourde à charger. Cet outil la coupe en une grille de tuiles
   plus un aperçu réduit, écrit le tout dans site/images/map/ et
   met à jour site/data/map-config.json — après quoi il n'y a
   plus qu'à publier sur git.
   ...
   Comme outils/editor.html, c'est un outil de dépôt : ses textes
...
(() => {
  ...
  const LIMITE_ENVOI_OCTETS = 64e6; // aligné sur MAX_IMAGE (claude/map-api.js)
  ...
  async function saveToRepo() {
    ...
    for (const f of files) {
      ...
      log(`écrit : site/images/map/${result.dir}/${f.name} (${ko(f.blob.size)})`, "ok");
    }
  ...
  function blockIfNotLocal() {
    ...
    document.body.innerHTML = `
      <div class="served-warning">
        ...
        <p>Depuis le dossier <code>apres-monde/</code>, lance :</p>
        <pre>node claude/dev-server.js 8080</pre>
        <p>puis ouvre <a href="http://localhost:8080/outils/tiles.html">http://localhost:8080/outils/tiles.html</a></p>
