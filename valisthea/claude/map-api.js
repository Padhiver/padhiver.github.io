// Routes d'écriture des outils locaux de carte (outils/editor.html et
// outils/tiles.html). Elles n'existent que dans claude/dev-server.js : sur
...
function savePoints(body) {
  ...
  const target = path.join(siteRoot, "data", "map-points.json");
  let comment;
  try {
  ...
  console.log(`site/data/map-points.json enregistré (${parsed.points.length} repère(s)).`);
...
function saveTile(body, query) {
  ...
  const outDir = path.join(siteRoot, "images", "map", dir);
  fs.mkdirSync(outDir, { recursive: true });
  fs.writeFileSync(path.join(outDir, name), body);
  console.log(`site/images/map/${dir}/${name} écrit (${Math.round(body.length / 1024)} Ko).`);
  return { ok: true, path: `site/images/map/${dir}/${name}`, bytes: body.length };
}
...
function saveMapSource(body) {
  ...
  const target = path.join(siteRoot, "data", "map-config.json");
  const config = JSON.parse(fs.readFileSync(target, "utf8"));
  if (!Array.isArray(config.maps)) throw new Error('site/data/map-config.json ne contient pas de liste "maps"');

  const entry = config.maps.find((m) => m && m.id === mapId);
  if (!entry) throw new Error(`carte inconnue dans site/data/map-config.json : "${mapId}"`);
  ...
  console.log(`site/data/map-config.json mis à jour pour la carte "${mapId}".`);
